...
@section('content')
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            ...
            <a href="{{ route('admin.categories.create') }}" class="inline-flex items-center justify-center bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2.5 rounded-xl font-bold shadow-lg shadow-indigo-200 hover:-translate-y-0.5 transition-all gap-2">
        ...
        <div class="bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <thead>
                        <tr class="bg-gradient-to-r from-indigo-600 to-violet-600 text-white">
                            <th class="pl-8 pr-4 py-5 text-xs font-bold uppercase tracking-widest text-white/90">Nama Kategori</th>
                            <th class="px-6 py-5 text-xs font-bold uppercase tracking-widest text-center w-48 text-white/90">Jumlah Soal</th>
                            <th class="px-8 py-5 text-xs font-bold uppercase tracking-widest text-right w-40 text-white/90">Aksi</th>
                        </tr>
                    </thead>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($categories as $category)
                            <tr class="group hover:bg-indigo-50/30 transition-colors duration-200">
                                ...
                                {{-- Kolom 1: Nama Kategori (Tanpa Ikon) --}}
                                <td class="pl-8 pr-4 py-5 align-middle">
                                    <div class="flex flex-col justify-center h-full">
                                        <span class="font-bold text-slate-800 text-base block mb-1 group-hover:text-indigo-600 transition-colors">
                                            {{ $category->name }}
                                        </span>
                                ...
                                <td class="px-6 py-5 text-center align-middle">
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold bg-slate-100 text-slate-600 group-hover:bg-white group-hover:shadow-sm border border-transparent group-hover:border-slate-200 transition-all">
                            ...
                            <tr>
                                <td colspan="3" class="px-6 py-20 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <a href="{{ route('admin.categories.create') }}" class="text-indigo-600 font-bold text-sm hover:underline">
            ...
            @if($categories->count() > 0)
                <div class="bg-slate-50 px-8 py-4 border-t border-slate-100 flex justify-between items-center">
                    ...
                    {{-- <div class="scale-90">{{ $categories->links() }}</div> --}}
                </div>
            @endif
        </div>
