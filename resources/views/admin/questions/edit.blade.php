            ...
            <form action="{{ route('admin.questions.update', $question->id) }}" method="POST" id="questionForm">
                ...
                <div class="p-8 space-y-8">
                    <div class="space-y-6">
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Kategori
                                (Bab)</label>

                            <div class="relative group">
                                <div
                                    class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none transition-colors group-hover:text-indigo-600">
                                    <i
                                        class="bi bi-folder2-open text-slate-400 group-hover:text-indigo-500 transition-colors"></i>
                                </div>
                                ...
                                <div class="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none">
                                    <div
                                        class="w-8 h-8 flex items-center justify-center rounded-lg bg-white border border-slate-200 text-slate-400 group-hover:border-indigo-200 group-hover:text-indigo-600 transition-all shadow-sm">
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Teks
                                Indikator / Pertanyaan</label>
                            <div class="relative group">
                                <div class="absolute top-3.5 left-0 pl-4 flex pointer-events-none">
                                    <i
                                        class="bi bi-chat-quote-fill text-slate-400 group-hover:text-indigo-500 transition-colors"></i>
                                </div>
                    ...
                    <div>
                        ...
                        <div id="options-container" class="space-y-3">
                            @foreach ($question->options as $index => $option)
                                <div
                                    class="option-row group flex gap-3 items-center bg-slate-50 p-2 rounded-2xl border border-slate-200 hover:border-indigo-300 hover:bg-white hover:shadow-sm transition-all">
                                    <div
                                        class="w-10 h-10 flex-shrink-0 flex items-center justify-center bg-white rounded-xl border border-slate-200 text-slate-400 font-bold text-sm shadow-sm group-hover:border-indigo-100 group-hover:text-indigo-500">
                                        {{ $index + 1 }}
                                    </div>
                                    <div class="flex-grow">
                                    ...
                                    <div class="w-24 relative border-l border-slate-200 pl-3">
                                        <div
                                            class="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none text-slate-300">
                                            <i class="bi bi-star-fill text-[10px]"></i>
                                        </div>
                                        <input type="number" name="options[{{ $index }}][score]"
                                            value="{{ $option->score_value }}"
                                            class="w-full pl-6 pr-2 py-1 bg-slate-100 border-none rounded-lg text-sm font-bold text-center focus:ring-0 text-slate-600"
                                            required>
                                    </div>
