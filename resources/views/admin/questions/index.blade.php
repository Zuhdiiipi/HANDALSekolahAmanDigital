        ...
        <div class="space-y-8">
            ...
            @forelse ($questions->groupBy('category.name') as $categoryName => $categoryQuestions)
                @php
                    $colors = [
                        [
                            ...
                            'bg_header' => 'bg-indigo-50',
                            'text' => 'text-indigo-700',
                            'icon' => 'text-indigo-500',
                        ],
                        [
                            'border' => 'border-rose-200',
                            'bg_header' => 'bg-rose-50',
                            'text' => 'text-rose-700',
                            'icon' => 'text-rose-500',
                        ],
                        [
                            'border' => 'border-amber-200',
                            'bg_header' => 'bg-amber-50',
                            'text' => 'text-amber-700',
                            'icon' => 'text-amber-500',
                        ],
                        [
                            'border' => 'border-emerald-200',
                            'bg_header' => 'bg-emerald-50',
                ...
                <div class="bg-white rounded-3xl shadow-sm border {{ $theme['border'] }} overflow-hidden">
                    <div
                        class="{{ $theme['bg_header'] }} px-8 py-5 flex justify-between items-center border-b {{ $theme['border'] }}">
                        <div class="flex items-center gap-3">
                            <i class="bi bi-folder2-open text-xl {{ $theme['icon'] }}"></i>
                            <h3 class="font-extrabold text-lg {{ $theme['text'] }}">{{ $categoryName }}</h3>
                        </div>
                        <span
                            class="px-3 py-1 bg-white/60 rounded-lg text-xs font-bold {{ $theme['text'] }} border border-white/50 backdrop-blur-sm">
                                        ...
                                        <div class="pl-8">
                                            <div class="flex gap-2 flex-wrap">
                                                @foreach ($q->options as $opt)
                                                    <div
                                                        class="inline-flex items-center rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-xs shadow-sm group-hover:border-slate-300 transition-colors">
                                                        <span
                                                            class="font-black {{ $theme['text'] }} mr-2 bg-slate-50 px-1.5 rounded">
                                                            {{ $opt->score_value }}
                                                        </span>
