...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
            ...
            <div
                class="flex items-center gap-3 bg-white px-5 py-2.5 rounded-full border border-slate-200 shadow-sm text-sm text-slate-600">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
            ...
            <div
                class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 border-l-[6px] border-l-blue-500 transition-transform hover:-translate-y-1 duration-300">
                ...
                @if ($stats['waiting_for_account'] > 0)
                    <a href="{{ route('admin.registrations.index') }}"
                        class="inline-flex items-center text-xs font-bold text-blue-600 hover:text-blue-800 mt-1">
                        Proses Data <i class="bi bi-arrow-right ml-1"></i>
                    </a>
                @else
                    <div
                        class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-700 text-xs font-bold mt-1">
            ...
            <div
                class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 border-l-[6px] border-l-emerald-500 transition-transform hover:-translate-y-1 duration-300">
                <div class="flex justify-between items-start mb-4">
                    <div>
            ...
            <div
                class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 border-l-[6px] border-l-orange-400 transition-transform hover:-translate-y-1 duration-300">
                <div class="flex justify-between items-start mb-4">
                    <div>
            ...
            <div
                class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 border-l-[6px] border-l-red-500 transition-transform hover:-translate-y-1 duration-300">
                <div class="flex justify-between items-start mb-4">
                    <div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2">
                <div class="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
                    <div class="px-8 py-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/30">
                        ...
                        <a href="{{ route('admin.registrations.index') }}"
                            class="text-xs font-bold text-blue-600 hover:text-blue-800 bg-blue-50 hover:bg-blue-100 px-4 py-2 rounded-xl transition-colors">
                            Lihat Semua
                        </a>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="w-full text-left text-sm">
                            <thead
                                class="bg-white text-slate-400 text-[11px] uppercase font-bold tracking-wider border-b border-slate-100">
                                <tr>
                                    ...
                                    <tr class="hover:bg-slate-50/50 transition-colors group">
                                        <td class="pl-8 pr-4 py-4">
                                            <div class="flex items-center gap-4">
                                                <div
                                                    class="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 text-white flex items-center justify-center font-bold text-sm shadow-sm">
                                                ...
                                                <div>
                                                    ...
                                                    <div class="flex items-center gap-2 mt-0.5">
                                                        <span
                                                            class="text-[10px] font-mono text-slate-400 bg-slate-100 px-1.5 rounded">{{ $reg->npsn }}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-4 py-4">
                                            <div class="flex flex-col">
                                                <span
                                                    class="text-xs font-semibold text-slate-600">{{ $reg->created_at->format('d M Y') }}</span>
                                                <span
                                                    class="text-[10px] text-slate-400">{{ $reg->created_at->diffForHumans() }}</span>
                                            </div>
                                        </td>
                                        <td class="px-4 py-4 text-right pr-8">
                                            @php
                                                ...
                                                $config = $statusConfig[$reg->status] ?? [
                                                    'bg' => 'bg-slate-50',
                                                    'text' => 'text-slate-600',
                                            ...
                                            @endphp
                                            <span
                                                class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-bold border {{ $config['bg'] }} {{ $config['text'] }} {{ $config['border'] }}">
                                    ...
                                    <tr>
                                        <td colspan="3" class="px-6 py-16 text-center text-slate-400">
                                            <div class="flex flex-col items-center justify-center">
                                                <div
                                                    class="w-12 h-12 bg-slate-50 rounded-full flex items-center justify-center mb-2">
            ...
            <div class="lg:col-span-1">
                <div
                    class="bg-slate-900 rounded-3xl shadow-xl shadow-slate-200 overflow-hidden text-white h-full relative border border-slate-800">

                    <div class="absolute -top-24 -right-24 w-64 h-64 bg-blue-600 rounded-full blur-[80px] opacity-20"></div>
                    <div class="absolute -bottom-24 -left-24 w-64 h-64 bg-purple-600 rounded-full blur-[80px] opacity-20">
                    </div>
                    ...
                    <div class="relative z-10">
                        <div class="px-6 py-6 border-b border-white/10 flex items-center justify-between">
                            ...
                            <a href="{{ route('ranking.page') }}"
                                class="text-[10px] font-bold text-slate-400 hover:text-white bg-white/5 hover:bg-white/10 px-3 py-1.5 rounded-lg transition-all">LIHAT
                                SEMUA</a>
                        </div>
                        ...
                        <div class="p-4 space-y-2">
                            @forelse($topSchools as $index => $school)
                                <div
                                    class="flex items-center p-3 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 hover:border-white/10 hover:scale-[1.02] transition-all cursor-default group">
                                    <div class="relative w-10 h-10 flex items-center justify-center">
                                        @if ($index == 0)
                                            <i
                                                class="bi bi-star-fill text-yellow-400 text-xs absolute -top-1 -right-1 animate-spin-slow"></i>
                                        @endif
                                        <div
                                    ...
                                    <div class="ml-3 flex-1 min-w-0">
                                        <p
                                            class="text-sm font-bold truncate text-slate-200 group-hover:text-white transition-colors">
                                            {{ $school->name }}</p>
                                    ...
                                    <div class="flex flex-col items-end">
                                        ...
                                        <span
                                            class="font-mono font-bold text-emerald-400">{{ number_format($school->current_score ?? 0, 0) }}</span>
                                    </div>
                                </div>
                            @empty
