    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div class="text-center mb-12 relative">
            ...
            <p class="text-slate-500 font-medium text-lg">
                ...
                <span
                    class="inline-block font-bold text-yellow-700 bg-yellow-50 px-3 py-0.5 rounded-lg border border-yellow-200 shadow-sm ml-1">
                    {{ $survey->year ?? date('Y') }}
                </span>
            </p>
        </div>
                    ...
                    <div class="flex flex-col items-center md:items-end">
                        ...
                        <div class="group/card relative flex items-center gap-5 bg-white/10 backdrop-blur-md px-8 py-5 rounded-2xl border {{ $theme['border'] }} shadow-lg min-w-[280px] cursor-default transition-all duration-500 ease-out hover:-translate-y-2 hover:scale-[1.03] {{ $theme['anim_class'] }} overflow-hidden">
                            ...
                            <div class="text-left relative z-10">
                                <h3
                                    class="text-4xl font-black uppercase tracking-tight {{ $theme['text_main'] }} drop-shadow-sm leading-none group-hover/card:scale-105 transition-transform duration-300 origin-left">
                                    {{ $survey->rank_label ?? 'N/A' }}
                                </h3>
                                <div
                                    class="h-1 w-10 bg-white/40 rounded-full mt-2 group-hover/card:w-20 transition-all duration-500">
                                </div>
                                <p
                                    class="{{ $theme['text_main'] }} text-[10px] font-bold mt-1.5 uppercase tracking-wider opacity-90">
                                    Level Keamanan
                                </p>
                            </div>
                        </div>
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden mt-8">
            <div class="px-8 py-6 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-4 bg-slate-50/50">
                ...
                <a href="{{ route('school.dashboard') }}"
                    class="text-sm font-bold text-slate-500 hover:text-blue-600 transition-colors flex items-center gap-1">
                ...
                @forelse($survey->answers as $answer)
                    <div class="p-8 hover:bg-slate-50/50 transition-colors">
                        <div class="flex gap-4 mb-4">
                            <span
                                class="flex-shrink-0 w-8 h-8 rounded-lg bg-slate-100 text-slate-500 flex items-center justify-center font-bold text-xs">
                                {{ $loop->iteration }}
                            </span>
                        ...
                        <div class="ml-12">
                            @php
                                $selectedOption = $answer->question->options
                                    ->where('id', $answer->answer_value)
                                    ->first();
                            @endphp
                            <div class="flex flex-col sm:flex-row sm:items-center gap-3">
                                <div
                                    class="inline-flex items-center gap-3 px-4 py-3 bg-blue-50/50 rounded-xl border border-blue-100 text-blue-800 w-full sm:w-auto">
                                    <div
                                ...
                                @if ($answer->validator_note)
                                    <div
                                        class="flex items-start gap-2 text-orange-600 bg-orange-50 px-4 py-2.5 rounded-xl border border-orange-100 text-xs font-medium max-w-md">
                                        <i class="bi bi-exclamation-circle-fill mt-0.5"></i>
                                        <span>
                                            <strong class="block mb-0.5 uppercase tracking-wide text-[10px]">Catatan
                                                Validator:</strong>
