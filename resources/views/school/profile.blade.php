...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    {{-- HEADER SECTION --}}
    <div class="relative bg-gradient-to-r from-blue-600 to-indigo-700 rounded-3xl p-8 mb-10 shadow-xl overflow-hidden text-white">
                ...
                <div class="p-8">
                    <form action="{{ route('school.profile.update') }}" method="POST">
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="bg-slate-50 p-5 rounded-2xl border border-slate-200 grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">NPSN</label>
                                    <div class="flex items-center gap-3 text-slate-600 font-bold bg-white px-4 py-3 rounded-xl border border-slate-200 shadow-sm">
                                ...
                                <div>
                                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Nama Sekolah</label>
                                    <div class="flex items-center gap-3 text-slate-600 font-bold bg-white px-4 py-3 rounded-xl border border-slate-200 shadow-sm">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <select name="jenjang" 
                                        ...
                                        <option value="SMA" {{ $school->jenjang == 'SMA' ? 'selected' : '' }}>Sekolah Menengah Atas (SMA)</option>
                                        <option value="SMK" {{ $school->jenjang == 'SMK' ? 'selected' : '' }}>Sekolah Menengah Kejuruan (SMK)</option>
                                        <option value="MA" {{ $school->jenjang == 'MA' ? 'selected' : '' }}>Madrasah Aliyah (MA)</option>
                                    </select>
                                </div>
                                @error('jenjang') <p class="text-red-500 text-xs mt-1 pl-1">{{ $message }}</p> @enderror
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    @error('phone_number') <p class="text-red-500 text-xs mt-1 pl-1">{{ $message }}</p> @enderror
                                </div>
                            </div>

                            {{-- ALAMAT (OUTLINE TEGAS) --}}
                            <div>
                                ...
                                @error('address') <p class="text-red-500 text-xs mt-1 pl-1">{{ $message }}</p> @enderror
                            </div>

                        </div>

                        <div class="mt-8 pt-6 border-t border-slate-100 flex justify-end">
                            <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 rounded-xl shadow-lg shadow-blue-600/30 transition-all transform hover:-translate-y-1 flex items-center gap-2">
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-3xl shadow-sm border border-slate-200 p-6 md:p-8 sticky top-6">
                <div class="flex items-center gap-4 mb-6">
                    <div class="w-12 h-12 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center text-xl shadow-sm border border-emerald-200">
                ...
                <form action="{{ route('school.profile.password') }}" method="POST">
                    ...
                    <div class="space-y-5">
                        ...
                        {{-- Password Lama --}}
                        <div>
                            <label class="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">Password Saat Ini</label>
                            <div class="relative group">
                                ...
                                <button type="button" onclick="togglePassword('current_password')" class="absolute inset-y-0 right-0 pr-4 flex items-center text-slate-400 hover:text-emerald-600 transition-colors">
                                    <i class="bi bi-eye"></i>
                                </button>
                            </div>
                        ...
                        {{-- Password Baru --}}
                        <div>
                            <label class="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">Password Baru</label>
                            <div class="relative">
                                <input type="password" name="password" id="password" required
                                    class="w-full px-4 py-3 bg-white rounded-xl border border-slate-300 text-slate-700 font-medium
                                           focus:outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-600 hover:border-emerald-400 transition-all duration-200">
                                <button type="button" onclick="togglePassword('password')" class="absolute inset-y-0 right-0 pr-4 flex items-center text-slate-400 hover:text-emerald-600 transition-colors">
                                    <i class="bi bi-eye"></i>
                                </button>
                            </div>
                        ...
                        {{-- Konfirmasi Password --}}
                        <div>
                            <label class="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">Ulangi Password Baru</label>
                            <div class="relative">
                                <input type="password" name="password_confirmation" id="password_confirmation" required
                                    class="w-full px-4 py-3 bg-white rounded-xl border border-slate-300 text-slate-700 font-medium
                                           focus:outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-600 hover:border-emerald-400 transition-all duration-200">
                                <button type="button" onclick="togglePassword('password_confirmation')" class="absolute inset-y-0 right-0 pr-4 flex items-center text-slate-400 hover:text-emerald-600 transition-colors">
                                    <i class="bi bi-eye"></i>
                                </button>
                            </div>
                        </div>
                    ...
                    <div class="mt-8">
                        <button type="submit" class="w-full bg-slate-800 hover:bg-slate-900 text-white font-bold py-3 px-4 rounded-xl shadow-lg hover:shadow-xl transition-all flex justify-center items-center gap-2 transform hover:-translate-y-0.5">
