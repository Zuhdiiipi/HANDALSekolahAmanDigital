...
@section('content')
    <div class="max-w-5xl mx-auto px-4 py-8">
        ...
        <div class="mb-8">
            <a href="{{ route('validator.dashboard') }}" class="inline-flex items-center text-sm text-slate-500 hover:text-blue-600 transition-colors mb-4 group">
        ...
        @if($registration->admin_notes)
            <div class="mb-8 bg-orange-50 border border-orange-100 rounded-xl p-5 flex gap-4 shadow-sm relative overflow-hidden">
                ...
                <div>
                    ...
                    <p class="text-xs text-orange-600/80 mt-2 font-semibold uppercase tracking-wide">Harap periksa poin revisi di atas.</p>
                </div>
            </div>
        @endif
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {{-- KOLOM KIRI: DATA DETAIL (2/3 Lebar) --}}
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    ...
                    <div class="p-6 grid gap-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Nama Sekolah</label>
                                <p class="text-lg font-bold text-slate-800">{{ $registration->school_name }}</p>
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">NPSN</label>
                                <div class="inline-flex items-center gap-2 bg-slate-100 px-3 py-1 rounded-md text-slate-700 font-mono font-medium text-sm">
                            ...
                            <div class="flex items-start gap-3 p-3 rounded-xl bg-slate-50 border border-slate-100">
                                ...
                                <p class="text-slate-700 text-sm leading-relaxed">
                                    ...
                                    <span class="text-slate-500">
                                        Kel. {{ $registration->village }}, Kec. {{ $registration->district }}, {{ $registration->city }}
                                    </span>
                                </p>
                            </div>
                        </div>
            ...
            {{-- KOLOM KANAN: DOKUMEN & AKSI (1/3 Lebar) --}}
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-5">
                    ...
                    <div class="group relative block p-4 bg-slate-50 border border-slate-200 rounded-xl hover:border-blue-400 hover:bg-blue-50 transition-all cursor-pointer">
                        <a href="{{ asset('storage/' . $registration->assessment_letter) }}" target="_blank" class="flex items-center gap-4">
                            <div class="w-12 h-12 bg-red-100 text-red-500 rounded-lg flex items-center justify-center text-2xl shadow-sm group-hover:scale-110 transition-transform">
                                <i class="bi bi-file-earmark-pdf-fill"></i>
                            </div>
                            <div class="overflow-hidden">
                                <p class="text-sm font-bold text-slate-700 truncate group-hover:text-blue-700 transition-colors">Surat Permohonan.pdf</p>
                ...
                <div class="bg-white rounded-2xl shadow-lg border border-slate-100 p-6 sticky top-6">
                    ...
                    <div class="space-y-3">
                        <form action="{{ route('validator.approve', $registration->id) }}" method="POST">
                            @csrf
                            <button class="w-full bg-blue-600 hover:bg-blue-700 text-white py-3.5 px-4 rounded-xl font-bold shadow-blue-200 shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2">
                                <i class="bi bi-check-lg text-lg"></i>
                                Verifikasi & Setujui
                            </button>
                        </form>

                        <button onclick="openRejectModal()" class="w-full bg-white border-2 border-red-100 text-red-600 hover:bg-red-50 hover:border-red-200 py-3.5 px-4 rounded-xl font-bold transition-all flex items-center justify-center gap-2">
    ...
    <div id="rejectModal" class="hidden fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        ...
        <div class="flex min-h-full items-center justify-center p-4 text-center">
            <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all sm:w-full sm:max-w-lg border border-slate-100">
                        ...
                        <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left w-full">
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-slate-500 mb-4">
                                    Pendaftaran akan dikembalikan ke sekolah untuk diperbaiki. Jelaskan alasannya di bawah ini.
                                </p>
                                <form action="{{ route('validator.reject', $registration->id) }}" method="POST" id="rejectForm">
                                    @csrf
                ...
                <div class="bg-slate-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 gap-2">
                    <button type="button" onclick="document.getElementById('rejectForm').submit();" class="inline-flex w-full justify-center rounded-lg bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-500 sm:w-auto transition-colors">
                        Kirim Penolakan
                    </button>
                    <button type="button" onclick="closeRejectModal()" class="mt-3 inline-flex w-full justify-center rounded-lg bg-white px-3 py-2 text-sm font-semibold text-slate-900 shadow-sm ring-1 ring-inset ring-slate-300 hover:bg-slate-50 sm:mt-0 sm:w-auto transition-colors">
                        Batal
                    </button>
                </div>
            </div>
