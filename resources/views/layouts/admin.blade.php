...
<body class="bg-slate-50 antialiased flex flex-col md:flex-row min-h-screen relative text-slate-800">
    ...
    {{-- ======================================================== --}}
    {{-- 1. HEADER MOBILE (Hanya muncul di layar kecil) --}}
    {{-- ======================================================== --}}
    <header class="bg-slate-900 text-white p-4 flex justify-between items-center md:hidden sticky top-0 z-40 shadow-lg border-b border-slate-800">
        <div class="flex items-center gap-3">
            {{-- Catatan: Di header mobile saya biarkan tetap putih jika Anda mau, jika tidak hapus 'brightness-0 invert' di sini juga --}}
        ...
        <button onclick="toggleSidebar()" class="text-white hover:text-blue-400 transition-colors focus:outline-none p-1">
    ...
    {{-- ======================================================== --}}
    {{-- 3. SIDEBAR (Navigasi Utama - TEMA GELAP) --}}
    {{-- ======================================================== --}}
    <aside id="sidebar"
        ...
        {{-- HEADER SIDEBAR (LOGO & TEKS BERSUSUN ATAS-BAWAH) --}}
        {{-- Menggunakan flex-col dan text-center agar bersusun vertikal --}}
        <div class="py-8 px-6 border-b border-slate-800 bg-slate-900 relative flex flex-col items-center justify-center text-center">
            
            {{-- Tombol Close (Mobile Only) - Diposisikan absolute di pojok --}}
            <button onclick="closeSidebar()" class="md:hidden absolute top-4 right-4 text-slate-500 hover:text-white transition-colors">
                <i class="bi bi-x-lg text-lg"></i>
            </button>

            {{-- Logo (Di Atas) --}}
            <div class="mb-4">
                {{-- PERBAIKAN DI SINI: Saya menghapus class 'brightness-0 invert' agar logo asli terlihat --}}
                <img src="{{ asset('img/Handal 8.png') }}" 
                        class="h-14 w-auto drop-shadow-md hover:scale-105 transition-transform" 
                        alt="Logo Handal">
            </div>
            ...
            <div>
                ...
                {{-- Badge Admin Panel --}}
                <span class="inline-block px-3 py-1 rounded-full text-[10px] font-bold bg-slate-800 text-blue-400 uppercase tracking-widest border border-slate-700 shadow-sm">
                    Admin Panel
                </span>
            </div>
        </div>
            ...
            @if (Auth::user()->role === 'validator')
                ...
                <a href="{{ route('validator.dashboard') }}"
                   ...
                   {{ request()->routeIs('validator.dashboard') ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/50' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
                    <i class="bi bi-speedometer2 text-lg {{ request()->routeIs('validator.dashboard') ? 'text-white' : 'group-hover:text-blue-400' }}"></i> 
                    <span>Dashboard</span>
                </a>
            @endif
            ...
            @if (Auth::user()->role === 'admin')
                ...
                <a href="{{ route('admin.dashboard') }}"
                   ...
                   {{ request()->routeIs('admin.dashboard') ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/50' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
                    <i class="bi bi-grid-1x2-fill text-lg {{ request()->routeIs('admin.dashboard') ? 'text-white' : 'group-hover:text-blue-400' }}"></i> 
                    <span>Dashboard</span>
                </a>
                ...
                <a href="{{ route('admin.registrations.index') }}"
                   ...
                   {{ request()->routeIs('admin.registrations.*') ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/50' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
                    <i class="bi bi-person-check-fill text-lg {{ request()->routeIs('admin.registrations.*') ? 'text-white' : 'group-hover:text-blue-400' }}"></i> 
                    <span>Penerbitan Akun</span>
                </a>

                {{-- Group: Manajemen Survei --}}
                <div class="px-3 mt-8 mb-3 text-[11px] font-bold text-slate-500 uppercase tracking-widest">Manajemen Data</div>
                ...
                <a href="{{ route('admin.categories.index') }}"
                   ...
                   {{ request()->routeIs('admin.categories.*') ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/50' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
                    <i class="bi bi-tags-fill text-lg {{ request()->routeIs('admin.categories.*') ? 'text-white' : 'group-hover:text-purple-400' }}"></i> 
                ...
                <a href="{{ route('admin.questions.index') }}"
                   ...
                   {{ request()->routeIs('admin.questions.*') ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/50' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
                    <i class="bi bi-question-circle-fill text-lg {{ request()->routeIs('admin.questions.*') ? 'text-white' : 'group-hover:text-purple-400' }}"></i> 
        ...
        <div class="p-4 border-t border-slate-800 bg-slate-900/50">
            ...
            <div class="flex items-center gap-3 mb-4 px-2">
                <div class="w-9 h-9 rounded-full bg-gradient-to-tr from-blue-500 to-cyan-400 flex items-center justify-center text-white font-bold text-sm shadow-md">
            ...
            <form action="{{ route('logout') }}" method="POST">
                @csrf
                <button class="w-full flex items-center justify-center gap-2 px-4 py-2.5 text-xs font-bold text-red-400 bg-red-500/10 hover:bg-red-500 hover:text-white rounded-lg transition-all duration-200 group">
